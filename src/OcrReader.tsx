  ...
  ({ onParse, onError, onStart, compact = false }, ref) => {
    ...
    return (
      <div className={compact ? "mt-2 space-y-2 text-xs text-slate-400" : "my-4 space-y-3"}>
        ...
        />
        {showProgress ? (
          <div className="relative overflow-hidden rounded-3xl border border-emerald-400/40 bg-emerald-500/10 p-4 shadow-[0_22px_60px_rgba(16,185,129,0.18)]">
            <div className="pointer-events-none absolute -inset-2 rounded-[32px] bg-gradient-to-r from-emerald-400/30 via-teal-300/20 to-sky-400/25 opacity-50 blur-3xl" />
            <div className="relative space-y-3">
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.32em] text-emerald-100/80">
                <span>Analyzing</span>
                <span className="text-xs text-emerald-50">{`${percentage}%`}</span>
              </div>
              <div className="relative h-2 overflow-hidden rounded-full bg-emerald-400/15">
                <div
                  className={`h-full rounded-full bg-gradient-to-r from-emerald-300 via-teal-300 to-sky-400 ${
                    barWidth > 0 ? "animate-shimmer-scan" : ""
                  }`}
              ...
              <div className="flex items-center gap-3 text-[11px] font-medium text-emerald-100">
                <span className="relative flex h-7 w-7 items-center justify-center">
                  <span className="absolute h-full w-full rounded-full border border-emerald-200/40" />
                  <span className="absolute h-full w-full rounded-full border-2 border-emerald-200/90 border-t-transparent animate-spin" />
                </span>
                <span className="flex-1 text-emerald-50/90">{stageLabel}</span>
              </div>
              {controllerActive && (
                <button
                  type="button"
                  onClick={() => abortControllerRef.current?.abort()}
                  className="text-xs font-semibold text-emerald-100 underline decoration-dotted underline-offset-2"
                >
                  Cancel
                </button>
              )}
